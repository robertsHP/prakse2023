<?php
    ...
    function saveAndUpdateToLocalDB ($data, $apiDataJSON) {
        $apiColumns = array_keys($data['columns']);
        $localColumns = array_values($data['columns']);
        $localTableName = $data['local-table-name'];
    ...
    function sendToAPI ($data, $apiDataJSON) {
        $apiColumns = array_keys($data['columns']);
        $localColumns = array_values($data['columns']);
        $localTableName = $data['local-table-name'];
        ...
        try {
            ...
            for ($i = 0; $i < count($rows); $i++) {
                if(!array_key_exists($i, $apiDataJSON)) {
                    ...
                    $response = POST($data['api-table-name'], $rowSwapped, $apiColumns);
                    if(!empty($response)) {
